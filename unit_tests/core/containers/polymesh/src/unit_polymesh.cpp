...
void polymesh_compile_test()
{
  ...
  typedef OpenTissue::trimesh::TriMesh<> mesh_type2;
  mesh_type2   m_mesh2;
  ...
  {
    mesh_type::edge_iterator e = m_mesh.edge_begin();
    while(e!=m_mesh.edge_end() && !edge_flip(*e)) ++e;
  }
  {
    mesh_type::face_iterator f0 = m_mesh.face_begin();
    mesh_type::face_iterator f1 = f0->get_border_halfedge_iterator()->get_twin_iterator()->get_face_iterator();
    face_merge(*f0,*f1);
  }
  {
    mesh_type::face_iterator f = m_mesh.face_begin();
    while(f!=m_mesh.face_end() && valency(*f)==3 ) ++f;

    mesh_type::vertex_iterator v0 = f->get_border_halfedge_iterator()->get_destination_iterator();
    mesh_type::vertex_iterator v1 = f->get_border_halfedge_iterator()->get_next_iterator()->get_next_iterator()->get_destination_iterator();
    face_split(*f,*v0,*v1);
  }
  {
    mesh_type::face_iterator f = m_mesh.face_begin();
  ...
  OpenTissue::polymesh::laplacian_smooth(m_mesh,.5);
  OpenTissue::polymesh::taubin_smooth(m_mesh,1);

  {
    ...
    if(!is_manifold(m_mesh))
    {
      mesh_type manifold;
      OpenTissue::mesh::remove_redundant_vertices(m_mesh,manifold);
      if(is_manifold(manifold))
      {
        OpenTissue::mesh::default_write(filename,manifold);
      }
    }
  }
  ...
  OpenTissue::polymesh::reflex_convex_decomposition(m_mesh,m_pieces);
...

  {
    ...
    for(;face!=fend;++face)
    {
      ...
      OpenTissue::polymesh::compute_face_normal(*face,n);
    }
  }
...
BOOST_AUTO_TEST_SUITE(opentissue_mesh_polymesh);

BOOST_AUTO_TEST_CASE(compile_testing)
{
  void (*ptr)() = &polymesh_compile_test;
  ptr = 0;
}
