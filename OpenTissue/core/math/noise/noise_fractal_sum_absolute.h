#ifndef OPENTISSUE_CORE_MATH_NOISE_NOISE_FRACTAL_SUM_ABSOLUTE_H
#define OPENTISSUE_CORE_MATH_NOISE_NOISE_FRACTAL_SUM_ABSOLUTE_H
//
    ...
    public:
      ...
      real_type operator()(real_type const & x,real_type const & y) const
      {
        ...
        for(int i=0;i<m_octaves;++i)
        {
          ...
          sum += fraction*std::fabs( m_noise(scale*x,scale*y) );
        }
        return sum;
      }
      ...
      real_type operator()(real_type const & x,real_type const & y,real_type const & z) const
      {
        ...
        for(int i=0;i<m_octaves;++i)
        {
          int scale = (1<<i);
          real_type fraction = (1.0/scale);
          sum += fraction*std::fabs( m_noise(scale*x,scale*y,scale*z) );
        }
        return sum;
      }
...
//OPENTISSUE_CORE_MATH_NOISE_NOISE_FRACTAL_SUM_ABSOLUTE_H
#endif
